...
describe('Pagination', () => {
    ...
    it('should return UNPROCESSABLE_ENTITY when query is invalid', async () => {
        ...
        await request(app.getHttpServer()).get('').query({ age: 20 }).expect(HttpStatus.UNPROCESSABLE_ENTITY);
    });

    it('should return entities which meet filter in query regardless of pagination type', async () => {
        const { body: cursorResponseBody } = await request(app.getHttpServer()).get('/').query({ name: 'name-9' }).expect(HttpStatus.OK);
    ...
    it('should be calculate the number of entities', async () => {
        ...
        } = await request(app.getHttpServer()).get('/').query({}).expect(HttpStatus.OK);
    ...
    describe('Offset', () => {
        ...
        it('should be keep query condition to next request', async () => {
            const promise = () => request(app.getHttpServer()).post('/').send({ name: 'same name' }).expect(HttpStatus.CREATED);
            for (let i = 0; i < 20; i++) {
        ...
        it('should be able to set limit only at first request', async () => {
            const limit = 15;
            const { body: responseBody } = await request(app.getHttpServer()).get('/').query({ limit }).expect(HttpStatus.OK);
        ...
        it('should be calculate the number of entities', async () => {
            const {
                body: { metadata: metadataAll },
            } = await request(app.getHttpServer()).get('/').query({ limit: 15 }).expect(HttpStatus.OK);
            expect(metadataAll.total).toEqual(totalCount);

            const {
                body: { metadata },
            } = await request(app.getHttpServer()).get('/').query({ name: 'name-1' }).expect(HttpStatus.OK);
            expect(metadata.total).toEqual(1);
        });
